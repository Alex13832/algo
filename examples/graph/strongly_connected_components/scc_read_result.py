...
edge_trace_res = go.Scatter(x=edge_res_x, y=edge_res_y, line=dict(width=3.5, color='#888'), hoverinfo='none',
                            mode='lines')

node_trace_res = go.Scatter(x=node_res_x, y=node_res_y, mode='markers', hoverinfo='text',
                            marker=dict(showscale=False,
                                        colorscale='mygbm',
                                        reversescale=True, color=component,
                                        size=12, colorbar=dict(thickness=15, xanchor='left', titleside='right'),
                                        line_width=1))

# Create network graph

fig = go.Figure(data=[edge_trace_in, node_trace_in, node_trace_res],
                layout=go.Layout(
                    title='',
                    titlefont_size=16,
                    showlegend=False,
