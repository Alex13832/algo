    ...
    for (auto dictWord: dict) {
      ...
      while (pos != string::npos) {    // Found!
        if (dictWord.length() > 2) {
          ...
          if (mode == isRow) {
            ...
            w.colStart = pos;
            w.rowEnd = count;
            w.colEnd = pos + dictWord.length() - 1;
          } else if (mode == isCol) {
            w.rowStart = pos;
            w.colStart = count;
            w.rowEnd = pos + dictWord.length() - 1;
            w.colEnd = count;
          } else if (mode == isDiag) {
            if (count >= height) {
      ...
      while (posR != string::npos) {    // Found!
        if (dictWord.length() > 2) {
          ...
          if (mode == isRow) {
            w.rowStart = count;
            w.colStart = width - posR - 1;
            w.rowEnd = count;
            w.colEnd = width - posR - dictWord.length();
          } else if (mode == isCol) {
            w.rowStart = height - posR - 1;
            w.colStart = count;
            w.rowEnd = height - posR - dictWord.length();
            w.colEnd = count;
          } else if (mode == isDiag) {
            if (count >= height) {
