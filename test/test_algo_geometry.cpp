...
TEST(Circle, EnclosingTriangle)
{
  Points points{{0.162745, 0.676737}, {0.578652, 0.674331},
                {0.201865, 0.252038}, {0.609536, 0.232788},
                {0.351138, 0.478224}, {0.365551, 0.491458}};
  Grid grid{points};
  auto circle = grid.MinEnclosingCircle();
  auto triangle = circle.EnclosingTriangle();

  auto res = std::all_of(points.begin(), points.end(),
                         [&](auto pt) { return triangle.IsInside(pt); });
...
{
  std::vector<Point> pts{{0, 0}, {5, 0}, {5, 5}, {0, 5}, {1, 2},
                         {2, 3}, {0, 3}, {2, 5}, {5, 4}};
  Polygon polygon{pts};
...
TEST(Grid, ConvexHullB)
{
  ...
  Points correct{{0.453056, 0.0932272}, {0.110242, 0.454161},
