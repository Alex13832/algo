...
constexpr auto Sign = [](auto x) { return -(x < 0); };

/// \brief Returns true if x is even.
constexpr auto IsEven = [](auto x) { return (x & 1) == 0; };

/// \brief Sets bit n in x to either true or false (1/0).
constexpr auto SetBit = [](auto x, auto n, bool mode) {
  if (mode) return (x | (1 << n));
  return (x & ~(1 << n));
};

/// \brief Tests if bit n in x is 1.
constexpr auto IsBit = [](auto x, auto n) { return (x & (1 << n)) > 0; };

/// \brief Changes a bit 0 to 1 or 1 to 0 at position n.
constexpr auto ToggleBit = [](auto x, auto n) { return (x ^ (1 << n)); };

/// \brief Returns true of x is a power of 2, e.g. 2^3 = 8.
constexpr auto IsPow2 = [](auto x) { return x && !(x & (x - 1)); };
...
constexpr auto SwapBits = [](auto x) {
  if (x == 0) return 1;
  if (x == 1) return 0;
  auto n = NextPow2(x);
  return x ^ (n - 1);
